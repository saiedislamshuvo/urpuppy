...
const MediaUploadSection = ({
  ...
  const [newFiles, setNewFiles] = reactExports.useState([]);
  const [isDeleting, setIsDeleting] = reactExports.useState(null);
  const [uploadKey, setUploadKey] = reactExports.useState(0);
  const defaultUrlsKey = reactExports.useMemo(() => sanitizedDefaultUrls.join(","), [sanitizedDefaultUrls]);
